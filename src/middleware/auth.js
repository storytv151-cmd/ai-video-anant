...
const authenticate = async (request, response, next) => {
  try {
    ...
    if (!token) {
      next(
        new ApiError(401, "Missing access token.", {
    ...
    if (user.accountStatus !== "active") {
      next(
        new ApiError(403, "User is not allowed to access this resource.", {
          code: "USER_SUSPENDED",
        }),
      );
      return;
    ...
    request.user = {
      ...
      adminRoleCode:
        user.role === "custom"
          ? user.metadata?.get?.("adminRoleCode") ||
            user.metadata?.adminRoleCode ||
            null
          : null,
    };
