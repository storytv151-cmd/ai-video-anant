...
const buildPublicProviderModelDto = (model) => ({
  ...
  limits: {
    maximumDuration: safeNumber(model.maximumDuration) ?? null,
    maximumResolution: model.maximumResolution || null,
  },
});
...
const buildPublicProviderDto = ({ provider, models = [], pricingSummary = [] }) => {
  ...
  return {
    ...
    capabilities: {
      ...
      limits: {
        maximumDuration: safeNumber(provider.maximumDuration) ?? null,
        maximumResolution: provider.maximumResolution || null,
      },
    },
    estimatedTimeMs,
