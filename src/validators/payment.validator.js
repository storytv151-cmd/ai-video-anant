...
const validateVerifyGooglePurchase = async (body = {}) => {
  ...
  if (
    ...
    !["credit_purchase", "subscription"].includes(body.paymentType)
  ) {
    errors.push({
      field: "paymentType",
      message: "paymentType must be credit_purchase or subscription.",
    });
  }
  if (body.paymentType === "subscription") {
    errors.push({
      field: "paymentType",
      message: "subscription verification is not implemented in this phase.",
    });
  }
  if (
    body.productType !== undefined &&
    !["inapp", "subs"].includes(body.productType)
  ) {
    errors.push({
      field: "productType",
      message: "productType must be inapp or subs.",
    });
  }
  if (body.productType === "subs") {
    errors.push({
      field: "productType",
      message: "subs is not implemented in this phase.",
    });
  }
...
const validateRestorePurchases = async (body = {}) => {
    ...
    if (!Array.isArray(body.purchases) || body.purchases.length === 0) {
      ...
      for (let i = 0; i < body.purchases.length; i += 1) {
        ...
        if (!item.purchaseToken || typeof item.purchaseToken !== "string") {
          errors.push({
            field: `purchases[${i}].purchaseToken`,
            message: "purchaseToken is required.",
          });
        }
        if (!item.productId || typeof item.productId !== "string") {
          errors.push({
            field: `purchases[${i}].productId`,
  ...
  if (!Array.isArray(body.purchases) && !hasLegacyArrays) {
    errors.push({
      field: "purchases",
      message: "Provide purchases or legacy productIds/purchaseTokens arrays.",
    });
  }
...
const validatePaymentHistoryQuery = async (query = {}) => {
  ...
  if (
    ...
    !["credit_purchase", "subscription"].includes(
      String(query.paymentType).trim().toLowerCase(),
    )
  ) {
    errors.push({
      field: "paymentType",
