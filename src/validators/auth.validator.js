...
const validateGoogleLogin = async (payload) => {
  ...
  if (!device || typeof device !== 'object') {
    ...
    if (!isNonEmptyString(device.platform) || !validPlatforms.includes(device.platform)) {
      errors.push({ field: 'device.platform', message: `Platform must be one of: ${validPlatforms.join(', ')}` });
    }

    if (!isNonEmptyString(device.deviceId) || device.deviceId.trim().length > 255) {
      errors.push({ field: 'device.deviceId', message: 'Device ID is required (max 255 chars).' });
    }

    if (device.appVersion !== undefined && device.appVersion !== null) {
      if (!isNonEmptyString(device.appVersion) || device.appVersion.trim().length > 50) {
        errors.push({ field: 'device.appVersion', message: 'App version must be a string (max 50 chars).' });
      }
    }

    if (device.osVersion !== undefined && device.osVersion !== null) {
      if (!isNonEmptyString(device.osVersion) || device.osVersion.trim().length > 50) {
        errors.push({ field: 'device.osVersion', message: 'OS version must be a string (max 50 chars).' });
      }
    }

    if (device.pushToken !== undefined && device.pushToken !== null) {
      if (!isNonEmptyString(device.pushToken) || device.pushToken.trim().length > 1024) {
        errors.push({ field: 'device.pushToken', message: 'Push token must be a string (max 1024 chars).' });
      }
    }

    if (device.model !== undefined && device.model !== null) {
      if (!isNonEmptyString(device.model) || device.model.trim().length > 120) {
        errors.push({ field: 'device.model', message: 'Device model must be a string (max 120 chars).' });
      }
    }
  }
  ...
  if (payload.idToken && typeof payload.idToken === 'string') {
    ...
    if (!tokenLike) {
      errors.push({ field: 'idToken', message: 'Google ID token format appears invalid.' });
    }
  }
...
const validateRefresh = async (payload) => {
  if (payload && typeof payload === 'object' && payload.refreshToken !== undefined && payload.refreshToken !== null) {
