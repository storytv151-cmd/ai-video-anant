...
const validateList = async (query = {}) => {
  ...
  if (query.minCredits !== undefined) {
    ...
    if (!Number.isFinite(min) || min < 0) {
      errors.push({ field: 'minCredits', message: 'minCredits must be a non-negative number.' });
    }
  }
  ...
  if (query.maxCredits !== undefined) {
    ...
    if (!Number.isFinite(max) || max < 0) {
      errors.push({ field: 'maxCredits', message: 'maxCredits must be a non-negative number.' });
    }
  }
