  ...
  if (Array.isArray(body.purchases)) {
    for (let index = 0; index < body.purchases.length; index += 1) {
      ...
      if (!item.purchaseToken || typeof item.purchaseToken !== 'string') {
        errors.push({ field: `purchases[${index}].purchaseToken`, message: 'purchaseToken is required.' });
      }
      if (item.packageName !== undefined && item.packageName !== null && typeof item.packageName !== 'string') {
        errors.push({ field: `purchases[${index}].packageName`, message: 'packageName must be a string.' });
      }
      if (item.productId !== undefined && item.productId !== null && typeof item.productId !== 'string') {
        errors.push({ field: `purchases[${index}].productId`, message: 'productId must be a string.' });
      }
    }
  }
  if (body.purchaseTokens !== undefined && !Array.isArray(body.purchaseTokens)) {
    errors.push({ field: 'purchaseTokens', message: 'purchaseTokens must be an array.' });
  }
  if (body.productIds !== undefined && !Array.isArray(body.productIds)) {
    errors.push({ field: 'productIds', message: 'productIds must be an array.' });
  }
  if (!Array.isArray(body.purchases) && !Array.isArray(body.purchaseTokens)) {
    errors.push({ field: 'purchases', message: 'Provide purchases or purchaseTokens for restore.' });
  }
  return errors.length > 0 ? buildResult({ valid: false, message: 'Validation failed.', errors }) : buildResult({ valid: true });
};

const validateSubscriptionStatusQuery = async () => buildResult({ valid: true });
...
const validateSubscriptionHistory = async (query = {}) => {
  ...
  if (query.sort !== undefined && !['asc', 'desc', 'oldest', 'latest'].includes(String(query.sort).trim().toLowerCase())) {
    errors.push({ field: 'sort', message: 'sort must be asc, desc, oldest, or latest.' });
  }
  return errors.length > 0 ? buildResult({ valid: false, message: 'Validation failed.', errors }) : buildResult({ valid: true });
};
