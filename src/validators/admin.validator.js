...
const validateRoleAssignmentBody = async (body = {}) => {
  ...
  if (body.adminRoleCode !== undefined && body.adminRoleCode !== null && typeof body.adminRoleCode !== 'string') {
    errors.push({ field: 'adminRoleCode', message: 'adminRoleCode must be a string.' });
  }
  return errors.length > 0
    ? buildResult({ valid: false, message: 'Validation failed.', errors })
    : buildResult({ valid: true });
...
const validateProviderPricingBody = async (body = {}) => {
  ...
  if (!Number.isFinite(Number(body.credits)) || Number(body.credits) < 0) {
    errors.push({ field: 'credits', message: 'credits must be zero or greater.' });
  }
  return errors.length > 0
    ? buildResult({ valid: false, message: 'Validation failed.', errors })
    : buildResult({ valid: true });
...
const validateSubscriptionActionBody = async (body = {}) => {
  const errors = [];
  if (body.planCode !== undefined && body.planCode !== null && typeof body.planCode !== 'string') {
