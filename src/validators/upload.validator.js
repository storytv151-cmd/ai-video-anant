...
const validateUploadBody = async (body = {}) => {
  ...
  if (
    ...
    body.ownerType !== null &&
    body.ownerType !== "" &&
    !isNonEmptyString(body.ownerType)
  ) {
...
const validateSignedUrlQuery = async (query = {}) => {
  ...
  if (
    operation &&
    !["upload", "download"].includes(String(operation).toLowerCase())
  ) {
    errors.push({
      field: "operation",
  ...
  if (String(operation || "").toLowerCase() === "download") {
    ...
    if (!isNonEmptyString(query.folder)) {
      errors.push({
        field: "folder",
        message: "folder is required for upload.",
      });
    }
    if (!isNonEmptyString(query.fileType)) {
      errors.push({
        field: "fileType",
        message: "fileType is required for upload.",
      });
    }
    if (!isNonEmptyString(query.mimeType)) {
      errors.push({
        field: "mimeType",
        message: "mimeType is required for upload.",
      });
    }
  }

  if (
    query.expiresInSeconds !== undefined &&
    query.expiresInSeconds !== null &&
    ...
    if (!Number.isFinite(n) || n <= 0 || n > 3600) {
      errors.push({
        ...
        message: "expiresInSeconds must be between 1 and 3600.",
      });
    }
  }
