...
app.use((request, response, next) => {
  ...
  const requestId =
    environment.app.enableRequestIdHeader && typeof incomingId === 'string' && validator.isUUID(incomingId)
      ? incomingId
      : uuidv4();
  ...
  runWithRequestContext(
    { requestId, ip: request.ip, path: request.originalUrl, method: request.method },
    () => next(),
  );
});
