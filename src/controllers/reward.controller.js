...
const claimDailyBonus = async (request, response) => {
  ...
  const data = await rewardService.claimDailyBonus({ userId: request.user.id, idempotencyKey: request.idempotencyKey });
...
const claimRewardAd = async (request, response) => {
  ...
  const data = await rewardService.claimRewardAd({ userId: request.user.id, idempotencyKey: request.idempotencyKey });
