...
const deleteFile = async (request, response) => {
  const data = await uploadService.deleteAsset({
    userId: request.user?.id,
...
const getSignedUrl = async (request, response) => {
  ...
  if (operation === "download") {
    ...
    const doc = await FileAssetModel.findOne({
      _id: fileId,
      isDeleted: false,
    }).lean();
    if (!doc) {
    ...
    const signed = await signedUrlService.generateSignedDownloadUrl({
  ...
  if (
    (normalizedFolder === "admin" || normalizedFolder.startsWith("admin/")) &&
    !adminContext.isAdmin
  ) {
    throw new ApiError(403, "Not authorized.", { code: "FORBIDDEN" });
  }
  if (
    (normalizedFolder === "system" || normalizedFolder === "logs") &&
    !adminContext.isAdmin
  ) {
    throw new ApiError(403, "Not authorized.", { code: "FORBIDDEN" });
  }
