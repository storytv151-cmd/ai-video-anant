...
const BLOCKED_EXTENSIONS = Object.freeze(new Set(['exe', 'dll', 'bat', 'cmd', 'sh', 'jar', 'com', 'msi', 'js']));
...
const assertAllowedFolder = (folder) => {
  ...
  if (!ALLOWED_FOLDERS.has(normalized)) {
    throw new ApiError(400, 'Folder is not allowed.', { code: 'STORAGE_FOLDER_NOT_ALLOWED', details: [{ folder: normalized }] });
  }
  return normalized;
};
...
const assertNotExecutable = (extension) => {
  ...
  if (ext && BLOCKED_EXTENSIONS.has(ext)) {
    throw new ApiError(400, 'Executable files are not allowed.', { code: 'STORAGE_EXECUTABLE_BLOCKED' });
  }
};
...
const assertSizeLimit = ({ sizeInBytes, maxBytes }) => {
  ...
  if (!Number.isFinite(size) || size < 0) {
    throw new ApiError(400, 'Invalid file size.', { code: 'UPLOAD_SIZE_INVALID' });
  }
  if (Number.isFinite(maxBytes) && maxBytes > 0 && size > maxBytes) {
    throw new ApiError(400, 'File size exceeds limit.', { code: 'UPLOAD_SIZE_LIMIT' });
  }
};
...
const assertAllowedMimeType = ({ mimeType, allowed }) => {
  ...
  if (!allowed.includes(mt)) {
    throw new ApiError(400, 'Unsupported file type.', { code: 'UPLOAD_UNSUPPORTED_TYPE', details: [{ mimeType: mt }] });
  }
};

const virusScanPlaceholder = async () => ({ scanned: false, clean: true });

const duplicateDetectionPlaceholder = async () => ({ checked: false, duplicate: false });
