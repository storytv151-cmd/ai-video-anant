...
const pickLimits = async ({ fileType }) => {
  ...
  const uploadLimits = settings?.uploadLimits || settings?.storageSettings?.uploadLimits || {};

  if (fileType === 'image') {
    const mb = Number(uploadLimits.maxImageSizeMB);
    return {
      maxBytes: Number.isFinite(mb) && mb > 0 ? mb * 1024 * 1024 : maxImageBytes,
  ...
  if (fileType === 'video') {
    ...
    return {
      maxBytes: Number.isFinite(mb) && mb > 0 ? mb * 1024 * 1024 : maxVideoBytes,
...
const isTemporaryFolder = (folder) => {
  ...
  return f === 'temporary' || f.endsWith('/temp') || f.endsWith('/temporary') || f.startsWith('temporary/');
};
...
const uploadImage = async ({ userId, file, body = {}, endpointType = 'image' } = {}) => {
  if (!userId) {
    throw new ApiError(401, 'Authentication required.', { code: 'AUTH_REQUIRED' });
  }
  if (!file?.buffer) {
    throw new ApiError(400, 'File is required.', { code: 'UPLOAD_FILE_REQUIRED' });
  }

  const folder = resolveFolderForEndpoint({ endpointType, folderOverride: body.folder });
  const ownerType = resolveOwnerType(body.ownerType);
  const ownerId = body.ownerId || null;

  const { maxBytes, allowedMimeTypes } = await pickLimits({ fileType: 'image' });
  storageValidationService.assertSizeLimit({ sizeInBytes: file.size, maxBytes });
  storageValidationService.assertAllowedMimeType({ mimeType: file.mimetype, allowed: allowedMimeTypes });
  ...
  const extension = storageValidationService.resolveExtensionForMimeType({ mimeType: processed.mimeType, fileType: 'image' }) || 'jpg';
  ...
  const storageKey = environment.integrations.digitalOceanSpaces.bucket
    ? digitalOceanSpaces.buildObjectKey({ prefix: folder, extension, isTemporary: isTemporaryFolder(folder) })
    : null;
  ...
  const uploaded = await storageService.uploadBuffer({
    ...
    cacheControl: endpointType === 'profile' ? 'private, max-age=3600' : 'public, max-age=31536000, immutable',
  });
  ...
  infoLogger.info('Upload image stored.', { fileId: String(doc._id), folder, storageKey: doc.storageKey });
  return buildResponse(doc);
};

const uploadVideo = async ({ userId, file, body = {}, endpointType = 'video' } = {}) => {
  if (!userId) {
    throw new ApiError(401, 'Authentication required.', { code: 'AUTH_REQUIRED' });
  }
  if (!file?.buffer) {
  ...
  const { maxBytes, allowedMimeTypes } = await pickLimits({ fileType: 'video' });
  storageValidationService.assertSizeLimit({ sizeInBytes: file.size, maxBytes });
  storageValidationService.assertAllowedMimeType({ mimeType: file.mimetype, allowed: allowedMimeTypes });
  await storageValidationService.virusScanPlaceholder();
  await storageValidationService.duplicateDetectionPlaceholder();
  ...
  const extension = storageValidationService.resolveExtensionForMimeType({ mimeType: file.mimetype, fileType: 'video' }) || 'mp4';
  storageValidationService.assertNotExecutable(extension);

  const storageKey = digitalOceanSpaces.buildObjectKey({ prefix: folder, extension, isTemporary: isTemporaryFolder(folder) });
  ...
  const meta = await videoStorageService.extractVideoMetadataPlaceholder(file.buffer);
  ...
  infoLogger.info('Upload video stored.', { fileId: String(doc._id), folder, storageKey: doc.storageKey });
  return buildResponse(doc);
};
...
const deleteAsset = async ({ userId, fileId } = {}) => {
  ...
  infoLogger.info('File deleted.', { fileId: String(doc._id), storageKey: doc.storageKey });
