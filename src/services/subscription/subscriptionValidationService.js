...
const assertFeatureName = ({ featureName, featureCatalog = [] } = {}) => {
  ...
  if (!normalized) {
    throw new ApiError(400, "featureName is required.", {
...
const assertSubscriptionNotExpired = (subscription = {}) => {
  ...
  if (!Number.isNaN(expiresAt.getTime()) && expiresAt.getTime() < Date.now()) {
    throw new ApiError(409, "Subscription has expired.", {
...
const isActiveStatus = (status) =>
  ACTIVE_STATUSES.includes(membershipService.normalizeStatus(status));

const assertTransitionSupported = ({
  transitionType,
  currentPlan,
  targetPlan = null,
} = {}) => {
  const normalized = String(transitionType || "")
    .trim()
    .toLowerCase();
  ...
  if (
    ...
    throw new ApiError(
      409,
      "Subscription transition is not allowed for this plan.",
      {
