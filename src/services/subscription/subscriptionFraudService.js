...
const assertPlanMatchesVerification = ({ plan, verification } = {}) => {
  ...
  if (verification.productId !== plan.googlePlayProductId) {
    throw new ApiError(
      400,
      "Subscription product does not match configured plan.",
      {
  ...
  if (
    ...
    plan.basePlanId !== verification.basePlanId
  ) {
    throw new ApiError(
      400,
      "Base plan does not match configured subscription plan.",
      {
...
const assertCountryAllowed = ({ plan, regionCode } = {}) => {
  ...
  if (
    ...
    !allowedCountries.includes(normalizedRegion)
  ) {
    throw new ApiError(
      400,
      "Subscription country is not allowed for this plan.",
      {
...
const assertOwnership = async ({
  ...
  if (purchaseTokenHash) {
    ...
    if (owner) {
      throw new ApiError(
        409,
        "Subscription purchase token is already linked to another user.",
        {
  ...
  if (externalAccountId) {
    ...
    if (owner) {
      throw new ApiError(
        409,
        "Subscription external account is already linked to another user.",
        {
  ...
  if (externalProfileId) {
    ...
    if (owner) {
      throw new ApiError(
        409,
        "Subscription external profile is already linked to another user.",
        {
...
const isNotificationAlreadyProcessed = ({
  subscription = {},
  notificationId = null,
} = {}) => {
  ...
  const history = Array.isArray(subscription?.history)
