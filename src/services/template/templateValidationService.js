...
const resolveProviderModelBySlug = async (slug) => {
  ...
  const model = await ProviderModelModel.findOne({
    slug: String(slug).toLowerCase(),
    enabled: true,
  }).lean();
  if (!model) {
...
const validateListQuery = (query) => {
  ...
  if (status && status !== "active") {
    throw new ApiError(
      400,
      "Only active templates are available in public APIs.",
      {
  ...
  if (
    requiredImages !== null &&
    (!Number.isFinite(requiredImages) ||
      requiredImages < 0 ||
      requiredImages > 20)
  ) {
    throw new ApiError(400, "Invalid requiredImages filter.", {
      code: "INVALID_REQUIRED_IMAGES",
    });
  }
  ...
  if (minCredits !== null && minCredits < 0) {
    throw new ApiError(400, "Invalid minCredits filter.", {
      code: "INVALID_MIN_CREDITS",
    });
  }
  if (maxCredits !== null && maxCredits < 0) {
    throw new ApiError(400, "Invalid maxCredits filter.", {
      code: "INVALID_MAX_CREDITS",
    });
  }
  if (minCredits !== null && maxCredits !== null && minCredits > maxCredits) {
    throw new ApiError(400, "minCredits cannot be greater than maxCredits.", {
      code: "INVALID_CREDITS_RANGE",
    });
  }
  ...
  return {
    ...
    providerModelSlug: query.providerModel
      ? String(query.providerModel).toLowerCase()
      : null,
