...
const createNotifications = async ({ payload = {}, adminUserId = null, request = null } = {}) => {
  const userIds = Array.isArray(payload.userIds) ? payload.userIds : payload.userId ? [payload.userId] : [];
  const targets =
    userIds.length > 0
      ? userIds
      : (
          await UserModel.find({ accountStatus: 'active' })
            .select({ _id: 1 })
            .limit(Math.min(adminQueryService.parsePositiveInt(payload.limit, 100), 500))
            .lean()
        ).map((user) => user._id);
