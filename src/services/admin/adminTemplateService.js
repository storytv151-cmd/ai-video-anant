...
const listTemplates = async ({ query = {} } = {}) => {
  ...
  let itemsQuery = VideoTemplateModel.find(filter)
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit);
  ...
  if (
    ...
    countQuery = VideoTemplateModel.countDocuments(filter).setOptions({
      withDeleted: true,
    });
  }
...
const createTemplate = async ({
  ...
  const template = await VideoTemplateModel.create(
    pickAllowedFields({ payload, fields: TEMPLATE_FIELDS }),
  );
...
const listCategories = async ({ query = {} } = {}) => {
  ...
  let itemsQuery = TemplateCategoryModel.find(filter)
  ...
  if (adminQueryService.parseBoolean(query.includeDeleted, false)) {
    itemsQuery = itemsQuery.withDeleted();
    countQuery = TemplateCategoryModel.countDocuments(filter).setOptions({
      withDeleted: true,
    });
  }
...
const updateCategory = async ({
  ...
  const category =
    await TemplateCategoryModel.findById(categoryId).withDeleted();
  if (!category) {
    throw new ApiError(404, "Category not found.", {
      code: "CATEGORY_NOT_FOUND",
    });
  ...
  if (payload.slug || payload.title) {
    category.slug = normalizeSlug(
      payload.slug || category.slug || payload.title,
    );
  }
  await category.save();
