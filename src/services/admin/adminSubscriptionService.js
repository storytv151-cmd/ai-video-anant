...
const findSubscriptionUser = async ({ userId, withDeleted = true } = {}) => {
  let query = UserModel.findById(userId).select({ name: 1, email: 1, country: 1, subscription: 1, role: 1, accountStatus: 1 });
  if (withDeleted) {
...
const listSubscriptions = async ({ query = {} } = {}) => {
  ...
  let findQuery = UserModel.find(filter)
    .select({ name: 1, email: 1, country: 1, subscription: 1, role: 1, accountStatus: 1 })
...
const cancelSubscription = async ({ userId, reason = null, adminUserId = null, request = null } = {}) => {
...
const removePremium = async ({ userId, reason = null, adminUserId = null, request = null } = {}) => {
