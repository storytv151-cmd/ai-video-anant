...
const assertCreditPurchaseRequest = (payload = {}) => {
  if (payload.paymentType && payload.paymentType !== "credit_purchase") {
    throw new ApiError(
      501,
      "Subscription verification is not implemented in this phase.",
      {
  ...
  if (payload.productType && payload.productType !== "inapp") {
    throw new ApiError(
      400,
      "Only one-time in-app products are supported in this phase.",
      {
...
const resolvePackageName = ({ payload, settings }) => {
  ...
  if (requested && allowed.length > 0 && !allowed.includes(requested)) {
    throw new ApiError(
      400,
      "Package name does not match configured Google Play application.",
      {
  ...
  if (!resolved) {
    throw new ApiError(500, "Google Play package name is not configured.", {
      code: "GOOGLE_PLAY_PACKAGE_NOT_CONFIGURED",
    });
  }
  return resolved;
};
...
const ensureVerifiableState = ({ verification, existingPayment = null }) => {
  const state = purchaseStateService.normalizePurchaseState(
  ...
  if (verification.isConsumed && !(existingPayment?.status === "success")) {
    throw new ApiError(
      409,
      "Purchase is already consumed and cannot be granted again.",
      {
...
const validateAgainstConfiguration = ({
  payload,
  verification,
  packageConfig,
}) => {
  if (!packageConfig) {
    throw new ApiError(
      400,
      "Requested product is not configured for purchase.",
      {
  ...
  if (packageConfig.productType !== "inapp") {
    throw new ApiError(
      400,
      "Only one-time credit packages are supported in this phase.",
      {
        code: "GOOGLE_PLAY_PRODUCT_TYPE_INVALID",
      },
    );
  ...
  if (verification.productId !== packageConfig.googlePlayProductId) {
    throw new ApiError(
      400,
      "Google Play product does not match configured package.",
      {
  ...
  const requestedOrderId = purchaseDuplicateProtectionService.normalizeString(
    payload.orderId,
  );
  if (
    requestedOrderId &&
    verification.orderId &&
    requestedOrderId !== verification.orderId
  ) {
    throw new ApiError(
      400,
      "Order ID does not match Google Play verification result.",
      {
  ...
  if (
    Array.isArray(packageConfig.countries) &&
    packageConfig.countries.length > 0 &&
    verification.regionCode
  ) {
    const allowedCountries = packageConfig.countries
      .map((item) => String(item).trim().toUpperCase())
      .filter(Boolean);
    if (
      allowedCountries.length > 0 &&
      !allowedCountries.includes(String(verification.regionCode).toUpperCase())
    ) {
      throw new ApiError(
        400,
        "Purchase country is not allowed for this package.",
        {
...
const maybeRunPostActions = async ({
  settings,
  verification,
  payment,
  requestMeta,
  ...
  if (
    ...
    purchaseStateService.canAcknowledgePurchase(verification.purchaseState)
  ) {
    try {
  ...
  if (
    settings?.googlePlay?.consumeOneTimePurchases &&
    !verification.isConsumed &&
    purchaseStateService.canConsumePurchase(verification.purchaseState)
  ) {
    try {
...
const verifyAndSettleCreditPurchase = async ({
  ...
  try {
    ...
    if (!settings.googlePlay?.allowCreditPurchases) {
      throw new ApiError(403, "Credit purchases are disabled.", {
        code: "GOOGLE_PLAY_CREDIT_PURCHASES_DISABLED",
      });
    }
    ...
    if (!packageConfig) {
      throw new ApiError(
        400,
        "Requested product is not configured for Google Play purchases.",
        {
          code: "GOOGLE_PLAY_PRODUCT_NOT_CONFIGURED",
    ...
    const purchaseTokenHash =
      purchaseDuplicateProtectionService.hashPurchaseToken(
        payload.purchaseToken,
      );
    const requestMeta = buildRequestMeta({ request, payload });

    const existingBeforeVerification =
      await purchaseDuplicateProtectionService.findExistingPayment({
        userId,
        purchaseTokenHash,
        orderId: purchaseDuplicateProtectionService.normalizeString(
          payload.orderId,
        ),
    ...
    if (
      existingBeforeVerification?.status === "success" &&
      existingBeforeVerification?.creditTransaction
    ) {
      return buildExistingResponse({
        existingPayment:
          existingBeforeVerification.toObject?.() || existingBeforeVerification,
      });
    }

    const verification =
      await googlePlayVerificationService.verifyInAppPurchase({
        packageName,
    ...
    const existingAfterVerification =
      await purchaseDuplicateProtectionService.findExistingPayment({
        userId,
        purchaseTokenHash,
    ...
    if (
      existingAfterVerification?.status === "success" &&
      existingAfterVerification?.creditTransaction
    ) {
      return buildExistingResponse({
        existingPayment:
          existingAfterVerification.toObject?.() || existingAfterVerification,
      });
    }
    ...
    ensureVerifiableState({
      verification,
      existingPayment:
        existingAfterVerification || existingBeforeVerification || null,
    });
    ...
    try {
      ...
      if (error?.code === 11000) {
        ...
        if (existingOnConflict) {
          return buildExistingResponse({
            existingPayment:
              existingOnConflict.toObject?.() || existingOnConflict,
          });
        }
      }
      throw error;
    ...
    const responsePayment =
      googlePostActions.updatedPayment?.toObject?.() ||
      googlePostActions.updatedPayment ||
      payment;
    ...
    return {
      ...
      creditTransactionId:
        settled?.transaction?._id ||
        responsePayment?.creditTransaction ||
...
const restoreCreditPurchases = async ({
  ...
  if (purchases.length === 0) {
    throw new ApiError(400, "At least one purchase is required for restore.", {
      code: "RESTORE_PURCHASES_REQUIRED",
    });
  }
