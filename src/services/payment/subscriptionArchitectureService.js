...
const getCurrentSubscription = async ({ userId }) => {
  ...
  const matchedPlan =
    plans.find(
      (plan) =>
        ...
        plan.googlePlayProductId === subscription.productId,
    ) || null;
  const mappedPlan = matchedPlan ? mapPlan(matchedPlan) : null;
  const premiumFeatures =
    Array.isArray(subscription.premiumFeatures) &&
    subscription.premiumFeatures.length > 0
  ...
  return {
    ...
    productId:
      subscription.productId || mappedPlan?.googlePlayProductId || null,
