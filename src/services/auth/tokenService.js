...
const buildJwt = ({ secret, payload, expiresInSeconds }) => {
  if (!secret) {
    throw new ApiError(500, 'JWT secret is not configured.', { code: 'JWT_SECRET_MISSING' });
  }
...
const verifyJwt = ({ secret, token, expectedType }) => {
  ...
  if (!safeEqual(signature, expectedSignature)) {
    throw new ApiError(401, 'Invalid token signature.', { code: 'TOKEN_INVALID' });
  }
  ...
  try {
    ...
    throw new ApiError(401, 'Invalid token payload.', { code: 'TOKEN_INVALID' });
  }
  ...
  if (expectedType && payload.typ !== expectedType) {
    throw new ApiError(401, 'Invalid token type.', { code: 'TOKEN_INVALID_TYPE' });
  }
...
const tokenService = Object.freeze({
  ...
  generateAccessToken: ({ userId, role }) => {
    const expiresInSeconds = parseDurationToSeconds(environment.security.jwtAccessExpires, 15 * 60);
    return buildJwt({
  ...
  verifyAccessToken: (token) =>
    verifyJwt({ secret: environment.security.jwtSecret, token, expectedType: 'access' }),
  verifyRefreshToken: (token) =>
    verifyJwt({ secret: environment.security.jwtRefreshSecret, token, expectedType: 'refresh' }),
});
