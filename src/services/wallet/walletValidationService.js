...
const assertEnoughPending = (wallet, amount) => {
  ...
  if (wallet.pendingCredits < amount) {
    throw new ApiError(409, "Insufficient pending credits.", {
      code: "INSUFFICIENT_PENDING_CREDITS",
    });
  }
};
...
const assertEnoughLocked = (wallet, amount) => {
  ...
  if (wallet.lockedCredits < amount) {
    throw new ApiError(409, "Insufficient locked credits.", {
      code: "INSUFFICIENT_LOCKED_CREDITS",
    });
  }
};
