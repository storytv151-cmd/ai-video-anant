...
const summarizeByFilter = async ({ filter = {} } = {}) => {
      ...
      VideoGenerationJobModel.aggregate([
        { $match: filter },
        {
          $group: {
            ...
            imagesGenerated: {
              $sum: buildCompletedOutputUnitsExpression("image"),
            },
            videosGenerated: {
              $sum: buildCompletedOutputUnitsExpression("video"),
            },
            audioGenerated: {
              $sum: buildCompletedOutputUnitsExpression("audio"),
            },
  ...
  const providers = providerIds.length
    ? await ProviderModel.find({ _id: { $in: providerIds } })
        .select({ name: 1, slug: 1 })
        .lean()
    : [];
  const providerMap = new Map(
    providers.map((provider) => [String(provider._id), provider]),
  );
  ...
  return {
    totals: {
      ...
      averageGenerationTimeMs: totals.averageGenerationTimeMs
        ? Math.round(Number(totals.averageGenerationTimeMs))
        : null,
      failureRate:
        totalJobs > 0 ? Number((failedJobs / totalJobs).toFixed(4)) : 0,
