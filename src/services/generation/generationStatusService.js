...
const getJobForUser = async ({ userId, jobId }) => {
  const job = await VideoGenerationJobModel.findOne({
    _id: jobId,
    user: userId,
  }).lean();
  if (!job) {
...
const buildProviderSummary = async ({ job }) => {
  ...
  const provider = await ProviderModel.findById(job.provider)
    .select({ name: 1, slug: 1, enabled: 1, healthStatus: 1 })
    .lean();
...
const getStatusResponse = async ({ userId, jobId }) => {
  ...
  const now = Date.now();
  const ect = job.estimatedCompletionTime
    ? new Date(job.estimatedCompletionTime).getTime()
    : null;
  if (
    ect &&
    Number.isFinite(ect) &&
    ...
    await walletService.withTransaction(async (session) => {
      const current = await VideoGenerationJobModel.findOne({
        _id: jobId,
        user: userId,
      }).session(session);
      if (!current) {
  ...
  if (
    ...
    Number(job.creditsUsed || 0) === 0 &&
    Number(job.costInCredits || 0) > 0
  ) {
    await walletService.withTransaction(async (session) => {
      const current = await VideoGenerationJobModel.findOne({
        _id: jobId,
      ...
      await VideoGenerationJobModel.updateOne(
        ...
        {
          $set: {
            creditsUsed: Number(current.costInCredits || 0),
