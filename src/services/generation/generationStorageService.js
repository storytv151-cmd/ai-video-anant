...
const isAllowedSpacesUrl = ({ urlString, bucket, endpoint, cdnBaseUrl }) => {
  ...
  const hostPrefix =
    u.host === endpointHost
      ? null
      : u.host.slice(0, u.host.length - (endpointHost.length + 1));
...
const assertUrlAllowed = ({ url, cdnBaseUrl, storageSettings }) => {
  ...
  const allowedCdn =
    storageSettings?.cdnBaseUrl ||
    cdnBaseUrl ||
    environment.integrations.digitalOceanSpaces.cdn ||
    null;
  ...
  if (environment.runtime.isProduction) {
    ...
    if (
      !isAllowedSpacesUrl({
        ...
        bucket,
        endpoint,
        cdnBaseUrl: allowedCdn,
      })
    ) {
      throw new ApiError(400, "Image URL is not allowed.", {
    ...
    throw new ApiError(
      400,
      "Image URL must use configured storage CDN base URL.",
      { code: "STORAGE_URL_INVALID" },
    );
  }
};
...
const validateMediaAssets = ({
  ...
  if (requiredCount !== null && items.length < requiredCount) {
    throw new ApiError(
      400,
      `${itemLabel} count is below the required minimum.`,
      { code: "INVALID_MEDIA_COUNT" },
    );
  }
  if (maxCount !== null && maxCount > 0 && items.length > maxCount) {
    throw new ApiError(400, `Too many ${itemLabel.toLowerCase()}s provided.`, {
      code: "UPLOAD_MEDIA_COUNT_LIMIT",
    });
  }
  ...
  for (let i = 0; i < items.length; i += 1) {
    const asset = items[i] || {};
    if (!asset.url) {
      throw new ApiError(400, `${itemLabel} URL is required.`, {
    ...
    if (asset.mimeType && String(asset.mimeType).trim() !== inferredMime) {
      throw new ApiError(400, "Invalid media MIME type.", {
    ...
    if (
      ...
      mimeToCheck &&
      !allowedMimeTypes.includes(mimeToCheck)
    ) {
    ...
    if (maxSizeBytes !== null) {
      const provided =
        asset.sizeInBytes !== null && asset.sizeInBytes !== undefined;
      if (environment.runtime.isProduction && !provided) {
        throw new ApiError(400, "Media size metadata is required.", {
      ...
      if (provided) {
        ...
        if (!Number.isFinite(size) || size < 0 || size > maxSizeBytes) {
          throw new ApiError(400, "Media size exceeds configured limit.", {
    ...
    if (environment.runtime.isProduction && !asset.storageKey) {
      throw new ApiError(400, "Media storageKey is required.", {
  ...
  return items.map((asset, idx) => ({
    ...
    mimeType:
      asset.mimeType || inferMimeTypeFromUrl(String(asset.url).trim()) || null,
...
const validateInputImages = ({
  ...
  const minRequired =
    minimumImages ??
    (Number.isFinite(Number(requiredImages)) ? Number(requiredImages) : null);
  const maxCount =
    maximumImages ??
    (Number.isFinite(Number(uploadLimits?.maxImageCount))
      ? Number(uploadLimits.maxImageCount)
      : null);
  const maxSizeBytes =
    ...
    Number(uploadLimits.maxImageSizeMB) > 0
      ...
      : null;
  const allowedMimeTypes = Array.isArray(uploadLimits?.allowedMimeTypes)
...
const validateInputVideos = ({ inputVideos, uploadLimits, storageSettings }) =>
  validateMediaAssets({
    ...
    requiredCount: null,
    maxCount:
      Number.isFinite(Number(uploadLimits?.maxVideoCount)) &&
      Number(uploadLimits.maxVideoCount) > 0
        ? Number(uploadLimits.maxVideoCount)
        : 5,
    maxSizeBytes:
      Number.isFinite(Number(uploadLimits?.maxVideoSizeMB)) &&
      Number(uploadLimits.maxVideoSizeMB) > 0
...
const validateInputAudio = ({ inputAudio, uploadLimits, storageSettings }) =>
  validateMediaAssets({
    ...
    requiredCount: null,
    maxCount:
      Number.isFinite(Number(uploadLimits?.maxAudioCount)) &&
      Number(uploadLimits.maxAudioCount) > 0
        ? Number(uploadLimits.maxAudioCount)
        : 5,
    maxSizeBytes:
      Number.isFinite(Number(uploadLimits?.maxAudioSizeMB)) &&
      Number(uploadLimits.maxAudioSizeMB) > 0
        ? Number(uploadLimits.maxAudioSizeMB) * 1024 * 1024
        : null,
    allowedMimeTypes: Array.isArray(uploadLimits?.allowedAudioMimeTypes)
      ? uploadLimits.allowedAudioMimeTypes
      : [],
    storageSettings,
    itemLabel: "Audio",
  });
