...
const shouldRefundOnFailure = (settings) => {
  const flag =
    generationValidationService.getNumericToggle(settings?.featureToggles, 'generationRefundOnFailure') ??
    generationValidationService.getNumericToggle(settings?.featureToggles, 'generation_refund_on_failure');
  if (flag === null) {
    return null;
  }
  return Boolean(flag);
...
const unlockIfLocked = async ({ userId, job, reason = 'Credits unlocked.', requestIdempotencyKey = null, session = null }) => {
...
const refundIfConsumed = async ({ userId, job, reason = 'Refund issued.', requestIdempotencyKey = null, session = null }) => {
  ...
  const refund = await walletService.refundCredits({
    ...
    description: reason,
    idempotencyKey: requestIdempotencyKey ? `gen_refund:${String(job._id)}:${String(requestIdempotencyKey)}` : null,
    session,
  });
...
const refundForFailure = async ({ settings, userId, job, reason, requestIdempotencyKey = null, session = null }) => {
  ...
  if (Number(job?.creditsUsed || 0) > 0) {
    return refundIfConsumed({ userId, job, reason, requestIdempotencyKey, session });
  }

  return unlockIfLocked({ userId, job, reason, requestIdempotencyKey, session });
};
...
const assertRefundEligibleStatus = (job) => {
  ...
  if (!['failed', 'cancelled', 'refunded'].includes(status)) {
    throw new ApiError(400, 'Job is not eligible for refund.', { code: 'JOB_REFUND_NOT_ELIGIBLE' });
  }
};
