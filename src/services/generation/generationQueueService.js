...
const enqueueJob = async ({ jobId, session } = {}) => {
  const queuePosition = await generationQueueProvider.getQueuePosition({ session });
  const query = VideoGenerationJobModel.updateOne({ _id: jobId }, { $set: { status: 'queued', queuePosition } });
  if (session) {
