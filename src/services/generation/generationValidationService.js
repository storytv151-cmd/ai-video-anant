...
const getGenerationSettings = async () => {
  ...
  return {
    ...
    mediaGenerationEnabled: Boolean(pick(system?.mediaGenerationEnabled, pick(system?.videoGenerationEnabled, true))),
...
const assertGenerationEnabled = (settings) => {
  ...
  if (!settings.mediaGenerationEnabled) {
    throw new ApiError(403, 'Media generation is disabled.', { code: 'VIDEO_GENERATION_DISABLED' });
  }
};
...
const normalizeTemplateMediaConfig = (template = {}) => {
  ...
  const maximumImages =
    Number.isFinite(Number(template.maximumImages))
  ...
  return {
    ...
    outputType: normalizeOutputType(template.outputType || deriveTemplateOutputType(template)),
    ...
    supportedOutputTypes:
      Array.isArray(template.supportedOutputTypes) && template.supportedOutputTypes.length > 0
        ? template.supportedOutputTypes.map((value) => normalizeOutputType(value)).filter(Boolean)
        : [normalizeOutputType(template.outputType || deriveTemplateOutputType(template))],
    defaultAspectRatio: template.defaultAspectRatio || template.aspectRatio || '16:9',
  };
};

const validateGenerationPayload = ({ template, payload = {} } = {}) => {
  const config = normalizeTemplateMediaConfig(template);
  const generationType = normalizeGenerationType(payload.generationType || config.generationType);
  const outputType = normalizeOutputType(payload.outputType || config.outputType);

  const inputImages = Array.isArray(payload.inputImages) ? payload.inputImages : [];
  const inputVideos = Array.isArray(payload.inputVideos) ? payload.inputVideos : [];
  const inputAudio = Array.isArray(payload.inputAudio) ? payload.inputAudio : [];
  const referenceImages = Array.isArray(payload.referenceImages) ? payload.referenceImages : [];
  const maskImages = Array.isArray(payload.maskImages) ? payload.maskImages : [];
  const prompt = payload.prompt ? String(payload.prompt).trim() : null;
  const negativePrompt = payload.negativePrompt ? String(payload.negativePrompt).trim() : null;
  const imageDrivenTypes = ['image_to_video', 'image_to_image', 'image_upscale', 'image_edit', 'background_remove', 'face_swap'];
  const videoDrivenTypes = ['video_to_video', 'video_upscale', 'video_edit'];

  const requirePrompt = generationType === 'text_to_image' || generationType === 'text_to_video' || config.allowPrompt;
  if (requirePrompt && !prompt && (generationType === 'text_to_image' || generationType === 'text_to_video')) {
    throw new ApiError(400, 'Prompt is required for this generation type.', { code: 'PROMPT_REQUIRED' });
  }

  if (imageDrivenTypes.includes(generationType)) {
    if (inputImages.length < Math.max(1, config.minimumImages)) {
      throw new ApiError(400, 'At least one input image is required.', { code: 'IMAGE_INPUT_REQUIRED' });
    }
  }

  if (Number.isFinite(Number(config.maximumImages)) && Number(config.maximumImages) >= 0 && inputImages.length > Number(config.maximumImages)) {
    throw new ApiError(400, 'Too many input images were provided.', { code: 'IMAGE_INPUT_LIMIT' });
  }

  if (videoDrivenTypes.includes(generationType) && inputVideos.length === 0) {
    throw new ApiError(400, 'At least one input video is required.', { code: 'VIDEO_INPUT_REQUIRED' });
  }

  if (generationType === 'audio_generation' && !prompt && inputAudio.length === 0) {
    throw new ApiError(400, 'Prompt or input audio is required.', { code: 'AUDIO_INPUT_REQUIRED' });
  }

  if (!config.allowInputVideo && !videoDrivenTypes.includes(generationType) && inputVideos.length > 0) {
    throw new ApiError(400, 'Input video is not allowed for this template.', { code: 'VIDEO_INPUT_NOT_ALLOWED' });
  }

  if (!config.allowInputAudio && generationType !== 'audio_generation' && inputAudio.length > 0) {
    throw new ApiError(400, 'Input audio is not allowed for this template.', { code: 'AUDIO_INPUT_NOT_ALLOWED' });
  }

  if (!config.allowReferenceImage && referenceImages.length > 0) {
    throw new ApiError(400, 'Reference images are not allowed for this template.', { code: 'REFERENCE_IMAGE_NOT_ALLOWED' });
  }

  if (!config.allowMaskImage && maskImages.length > 0) {
    throw new ApiError(400, 'Mask images are not allowed for this template.', { code: 'MASK_IMAGE_NOT_ALLOWED' });
  }

  if (!config.allowNegativePrompt && negativePrompt) {
    throw new ApiError(400, 'Negative prompt is not allowed for this template.', { code: 'NEGATIVE_PROMPT_NOT_ALLOWED' });
  }

  if (config.supportedOutputTypes.length > 0 && !config.supportedOutputTypes.includes(outputType)) {
    throw new ApiError(400, 'Output type is not supported by this template.', { code: 'OUTPUT_TYPE_NOT_SUPPORTED' });
  }

  if (payload.multipleOutputs && !config.allowMultipleOutputs) {
    throw new ApiError(400, 'Multiple outputs are not allowed for this template.', { code: 'MULTIPLE_OUTPUTS_NOT_ALLOWED' });
  }
  ...
  return {
    ...
    prompt,
    negativePrompt,
    multipleOutputs: Boolean(payload.multipleOutputs && config.allowMultipleOutputs),
